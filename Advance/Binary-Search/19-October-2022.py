...
class Solution:
    ...
    def binarySearch(self, stalls, cows):
        n = len(stalls)
        low = 1
        ...
        while (low <= high):
            ...
            if (self.check(stalls, cows, mid) == True):
                result = mid #if we can keep cows atleast mid distance apart
                low = mid + 1
            else:
                high = mid - 1
    ...
    def check(self, stalls, totalCows, mid):
        n = len(stalls)
